...
const Body = z.object({ text: z.string().min(1).max(1000), runId: z.string().max(40).optional() });
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const reply = await followup({ moonletId: id, owner, text: body.data.text, runId: body.data.runId ?? null });
