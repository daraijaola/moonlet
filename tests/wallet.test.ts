...
describe("wallet_activity (live RPC)", () => {
  it("scans a busy wallet in chunks under budget and groups by token", async () => {
    const [chain] = buildTools(["chain_read"], { delivery: {} }) as unknown as Array<{ function: { execute: (a: unknown) => Promise<unknown> } }>;
    // the ORBIO/NVDA Uniswap pool (the pool contract itself, seen in every swap)
    const t0 = Date.now();
    const r = (await chain.function.execute({ action: "wallet_activity", wallet: "0x8366a39cc670b4001a1121b8f6a443a643e40951", limit: 5 })) as Record<string, unknown>;
    ...
    const again = (await chain.function.execute({ action: "wallet_activity", wallet: "0x8366a39cc670b4001a1121b8f6a443a643e40951", fromBlock: Number(r.latestBlock), limit: 5 })) as Record<string, unknown>;
