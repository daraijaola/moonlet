...
describe("telegram concierge (real model, owner's key)", () => {
  ...
  it("answers about the latest result from the store, not from imagination", async () => {
    const r = await concierge(OWNER, "what did tide find last time?", { appUrl: "https://16labs.xyz" });
  ...
  it("changes cadence when asked in plain words", async () => {
    const r = await concierge(OWNER, "make tide run every 12 hours instead", { appUrl: "https://16labs.xyz" });
  ...
  it("runs now through the injected runner and says the result is coming", async () => {
    ...
    const r = await concierge(OWNER, "run it now please", { appUrl: "https://16labs.xyz", runNow: async (id) => { ran = id; } });
